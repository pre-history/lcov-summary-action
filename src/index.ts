...
async function main() {
    ...
    if (inputs.baseFile) {
        ...
        if (!baseRawCoverageReport)
            console.log(`No coverage report found at '${inputs.baseFile}', ignoring...`);
    }
    let options: Options = getCommitDetails(inputs);
}
function getInputs() {
    const lcovFile = getInputFilePath(core.getInput("lcov-file"), "./coverage/lcov.info");
...
function getInputBoolValue(inputName: string): boolean {
    return core.getInput(inputName).toLowerCase() === "true";
}
