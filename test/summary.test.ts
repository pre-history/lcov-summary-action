...
describe('integration tests', () => {
  it('should generate detailed summary with real LCOV data', () => {
    ...
    assert.ok(summary.includes('Test Coverage Report'), 'Should contain custom title');
    assert.ok(summary.includes('Generated by LCovMan 🦕'), 'Should contain LCovMan signature');
    assert.ok(summary.includes('Coverage Badge'), 'Should contain badge section');
    assert.ok(summary.includes('Copy this to your README.md'), 'Should contain badge instructions');
    assert.ok(summary.includes('50%'), 'Should contain coverage percentage');
    assert.ok(summary.includes('❌ 50%'), 'Should show threshold failure for 50% vs 70%');
  });

  it('should generate coverage comparison diff', () => {
    const currentLcov = fs.readFileSync('test/fixtures/50lcov.info.sample', 'utf8');
    ...
    assert.ok(diff!.percentage_diff > 0, 'Should show improvement from 0% to 50%');
    assert.equal(diff!.percentage_diff, 50, 'Should show exactly 50% improvement');
    ...
    assert.ok(summaryWithDiff.includes('Coverage Changes 📈'), 'Should show improvement emoji');
  ...
  it('should handle badge generation for different coverage levels', () => {
    const testCases = [
      { fixture: '0lcov.info.sample', expectedColor: 'red', expectedEmoji: '❌' },
      { fixture: '50lcov.info.sample', expectedColor: 'yellow', expectedEmoji: '⚠️' },
      { fixture: '100lcov.info.sample', expectedColor: 'brightgreen', expectedEmoji: '✅' }
    ];
    ...
    testCases.forEach(({ fixture, expectedColor, expectedEmoji }) => {
      ...
      assert.ok(summary.includes(expectedColor), `Should use ${expectedColor} for ${fixture}`);
      assert.ok(summary.includes('for-the-badge'), 'Should use correct badge style');
  ...
  it('should show file breakdown for detailed summary', () => {
    ...
    assert.ok(summary.includes('Files with Low Coverage'), 'Should show low coverage section');
    ...
    result.files.forEach(file => {
      ...
      assert.ok(typeof file.percentage === 'number', 'File should have percentage');
      assert.ok(typeof file.covered === 'number', 'File should have covered lines');
