...
describe('index.ts integration tests', () => {
  ...
  it('should validate badge styles correctly', () => {
    ...
    assert.ok(
      isValidBadgeStyle('flat-square'),
      'Should accept flat-square style',
    );
    assert.ok(
      isValidBadgeStyle('for-the-badge'),
      'Should accept for-the-badge style',
    );
    assert.ok(
      !isValidBadgeStyle('invalid-style'),
  ...
  it('should generate coverage badges correctly', () => {
    ...
    testCases.forEach(({ percentage, expectedColor }) => {
      ...
      assert.ok(
        badge.url.includes(expectedColor),
        `${percentage}% should use ${expectedColor}`,
      );
      assert.ok(
        badge.url.includes(`${percentage}%25`),
        'Should include percentage in URL',
      );
      assert.ok(
        badge.markdown.startsWith('![Coverage]'),
        'Should generate markdown format',
      );
      assert.ok(
        badge.url.includes('style=flat'),
        'Should include style parameter',
      );
    });
  });
  ...
  it('should parse LCOV files correctly', () => {
    ...
    assert.equal(
      result.not_covered,
      5,
      'Should parse uncovered lines correctly',
    );
  ...
  it('should generate summaries correctly', () => {
    ...
    assert.ok(
      summary.includes('#4CAF50'),
      'Should include custom primary color',
    );
    assert.ok(
      summary.includes('#FF5733'),
      'Should include custom secondary color',
    );
    assert.ok(
      summary.includes('Generated by LCovMan 🦕'),
      'Should include LCovMan signature',
    );
  });
  ...
  it('should generate detailed summaries with real data', () => {
    ...
    assert.ok(
      detailedSummary.includes('Integration Test Coverage'),
      'Should include custom title',
    );
    assert.ok(
      detailedSummary.includes('Generated by LCovMan 🦕'),
      'Should include signature',
    );
    assert.ok(
      detailedSummary.includes('Coverage Badge'),
      'Should include badge section',
    );
    assert.ok(
      detailedSummary.includes('flat-square'),
      'Should use specified badge style',
    );
    assert.ok(
